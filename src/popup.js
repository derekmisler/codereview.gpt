...
async function callChatGPT(messages, callback, onDone) {
  ...
  if (!apiKey) {
    callback(
      'Please add your OpenAI API key to the `OPENAI_API_KEY` variable within your .env file.'
    );
    onDone();
    return;
  }
...
async function reviewPR(diffPath, context, title) {
  ...
  // ChatGPT 4 has a maximum token size of 128,000 tokens https://platform.openai.com/docs/models/continuous-model-upgrades
  // We will use the guidance of 1 token ~= 4 chars in English, minus 1000 chars to be sure.
  // This means we have 128000, and let's reduce 1000 chars from that.
  const maxTokens = 127000;
  var files = parsediff(patch);

  // Rebuild our patch as if it were different patches
  files.forEach(function (file) {
    // Ignore lockfiles
    if (file.from.includes('lock.json') || file.from.includes('yarn.lock')) {
      return;
    }
    ...
    if (patchPart.length >= maxTokens) {
      patchPart = patchPart.slice(0, maxTokens);
      warning =
